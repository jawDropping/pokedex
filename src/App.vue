...
const pokemons = ref<Pokemon[]>([]) // paginated (non-search) view
const search = ref("")
...
const MAX_SEARCH_RESULTS = 30; // cap detail fetches so typing "a" doesn't fetch hundreds
...
watch(search, (term) => {
  ...
  }, 300); // debounce so we don't fetch on every keystroke
});

onMounted(async () => {
  loadMore(); // initial paginated batch
  allNames.value = await fetchAllPokemonNames(); // build search index in background
})

// What the grid actually displays
